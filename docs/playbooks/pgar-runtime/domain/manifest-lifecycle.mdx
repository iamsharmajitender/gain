...
[Blueprint](/blueprints/pgar-blueprint) · [← Manifest registry](/playbooks/pgar-runtime/domain/manifest-registry) · **Manifest lifecycle** · [RAG retrieval →](/playbooks/pgar-runtime/domain/rag-retrieval)

The [tool manifest](/playbooks/pgar-runtime/domain/manifest-registry) defines **what** tools exist and **how** proposals are enforced. This page covers **where** manifests live, **how** apps load them, and **which pattern** fits your scale.
...
Manifest **shape** and **enforcement** live in [Manifest registry](/playbooks/pgar-runtime/domain/manifest-registry). This page is **operations and architecture choice**.
...
RAG tools (`retrieve_documents`, etc.) use the same JSON shape as payment tools. See [Manifest registry § RAG tools](/playbooks/pgar-runtime/domain/manifest-registry#rag-tools-in-the-same-model).
...
3. **Derive LLM tools:** Strip `pdp_action`, `risk_tier`, entitlements ([Manifest registry § manifest → LLM](/playbooks/pgar-runtime/domain/manifest-registry#manifest--llm--pep)).
...
See: [Manifest registry](/playbooks/pgar-runtime/domain/manifest-registry) · [Boundary: Agentic app](/playbooks/pgar-runtime/boundary/agentic-app) · [Audit & replay](/playbooks/pgar-runtime/foundation/audit-and-replay)
