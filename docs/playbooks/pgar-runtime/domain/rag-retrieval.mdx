...
**Tool manifest:** version `2026.07.1`, held by agentic app. This walkthrough uses the **RAG tool subset**; manifest shape and enforcement hops are defined in [Manifest registry § full manifest](/playbooks/pgar-runtime/domain/manifest-registry#full-manifest-example) and [§ one proposal](/playbooks/pgar-runtime/domain/manifest-registry#worked-example-one-proposal).
...
The app derives the `tools` array from the manifest (see [Manifest registry § manifest → LLM](/playbooks/pgar-runtime/domain/manifest-registry#manifest--llm--pep)). Same three names; no `pdp_action`, `risk_tier`, or entitlements in the LLM payload.
