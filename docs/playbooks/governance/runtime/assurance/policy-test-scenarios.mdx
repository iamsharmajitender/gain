---
slug: /governance/runtime/assurance/policy-test-scenarios
...
[Blueprint](/blueprints/governance/runtime) · [← Audit & replay](/playbooks/governance/runtime/foundation/audit-and-replay) · **Policy scenarios** · [Adversarial →](/playbooks/governance/runtime/assurance/adversarial-testing)

Policy regressions are deterministic. Build a versioned scenario library parallel to [eval golden datasets](/playbooks/evaluation/golden-datasets).
...
<Details summary="Policy scenario schema (JSON)">
...
<Details summary="CI scenario harness (pseudo-code)">
...
Assurance runs at different depths across the delivery lifecycle. **Policy test scenarios** are the deterministic core (SARAC in, verdict out). **[Adversarial testing](/playbooks/governance/runtime/assurance/adversarial-testing)** adds bypass and injection paths on top.

**Teams** (aligned with [PGAR Blueprint ownership](/blueprints/governance/runtime#ownership)):
...
| **Online** | Trace eval overlap | AI platform (eval pipeline) | AI platform + Governance (thresholds) | [Action](/playbooks/evaluation/plane-action) / [Tool](/playbooks/evaluation/plane-tool) planes on traces | Dashboard: block rate, step-up rate, unknown tools |
...
<Details summary="CI commands (bash)">
...
Pair with the [PGAR release gate matrix](/blueprints/governance/runtime#release-gate-matrix) for which scenario layers to re-run per change type.
...
See: [Adversarial testing](/playbooks/governance/runtime/assurance/adversarial-testing) · [PDP policy surfaces](/playbooks/governance/runtime/foundation/pdp-policy-surfaces)
