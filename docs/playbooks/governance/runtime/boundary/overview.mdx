---
slug: /governance/runtime/boundary
...
[Runtime](/playbooks/governance/runtime) · [Blueprint](/blueprints/governance/runtime) · **Boundary overview** · [① Ingress →](/playbooks/governance/runtime/boundary/ingress)
...
| --- | --- | --- | --- |
| **①** | **Ingress** | Who is this principal, and are their claims valid? | [Ingress](/playbooks/governance/runtime/boundary/ingress) |
| **②** | **Agentic app** | Who holds the token, routes proposals, and initiates validation? | [Agentic app](/playbooks/governance/runtime/boundary/agentic-app) |
| **③** | **LLM proposal** | What did the model propose, with no authority or credentials? | [LLM proposal](/playbooks/governance/runtime/boundary/llm-proposal) |
| **④** | **PEP + PDP** | Does policy ALLOW, DENY, or require STEP_UP before any side effect? | [PEP + PDP](/playbooks/governance/runtime/boundary/pep-pdp) |
| **⑤** | **Downstream** | Does the target service re-auth and execute, returning results to the app? | [Downstream](/playbooks/governance/runtime/boundary/downstream) |

Read them **in order** when implementing a new agent surface. Foundations ([token boundary](/playbooks/governance/runtime/foundation/token-and-session-boundary), [PEP enforcement](/playbooks/governance/runtime/foundation/pep-enforcement), [policy contracts](/playbooks/governance/runtime/foundation/policy-contracts)) should be in place first.
...
The orchestrator holds session state and credentials, loads the [tool manifest](/playbooks/agents/manifests/manifest-lifecycle) selected at ingress by the [intent router](/playbooks/agents/intent-router), calls the LLM with schemas only, forwards proposals to the PEP, receives downstream results, runs validation (for RAG), and only then sends validated context back to the model.
...
1. Entry in the caller's [tool manifest](/playbooks/agents/manifests/manifest-lifecycle)
...
See [Agentic app](/playbooks/governance/runtime/boundary/agentic-app) for the single-agent request loop and [Policy contracts](/playbooks/governance/runtime/foundation/policy-contracts) for SARAC subject shapes under delegation.

## After the boundaries

PGAR gates side effects. The store or contract lives on the G.A.I.N subject series:

- [Agents: Manifests](/playbooks/agents/manifests/manifest-registry) for tool execute
- [RAG](/playbooks/rag) for governed context packs
- [Memory](/playbooks/agents/orchestration/memory) for prefs and episodes

Bridge: [Governance Blueprint](/blueprints/governance/runtime) · [Agents Blueprint](/blueprints/agents-blueprint) (manifests) · [MCP Blueprint](/blueprints/mcp-blueprint) (transport) · [RAG Blueprint](/blueprints/rag-blueprint)

## Read next

**[① Ingress →](/playbooks/governance/runtime/boundary/ingress)**
