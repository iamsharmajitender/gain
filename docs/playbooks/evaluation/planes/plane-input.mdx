...
[Blueprint](/blueprints/evaluation-blueprint) · **Input** · [Data →](/playbooks/evaluation/plane-data)

The Input plane is where requests enter: parsing, normalization, intent classification, and first-line safety filters. Failures here poison every downstream plane. See [Intent router playbooks](/playbooks/agents/intent-router) for the architectural role; this plane is where you eval it.
...
- **Intent misroute** — wrong workflow or tool family selected ([Intent router](/playbooks/agents/intent-router))
