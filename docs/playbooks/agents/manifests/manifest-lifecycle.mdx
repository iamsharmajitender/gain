---
slug: /agents/manifests/manifest-lifecycle
...
[Agents overview](/playbooks/agents) · [← Manifest registry](/playbooks/agents/manifests/manifest-registry) · **Manifest lifecycle**

The [tool manifest](/playbooks/agents/manifests/manifest-registry) defines **what** tools exist and **how** proposals are enforced. This page covers **where** manifests live, **how** apps load them, and **which pattern** fits your scale.
...
| **CI gate on change** | New tool = schema + [policy scenarios](/playbooks/governance/runtime/assurance/policy-test-scenarios) |
| **Audit field** | `manifest_version` on every proposal ([Audit & replay](/playbooks/governance/runtime/foundation/audit-and-replay)) |

Manifest **shape** and **enforcement** live in [Manifest registry](/playbooks/agents/manifests/manifest-registry). This page is **operations and architecture choice**.
...
RAG tools (`retrieve_documents`, etc.) use the same JSON shape as payment tools. See [Manifest registry § RAG tools](/playbooks/agents/manifests/manifest-registry#rag-tools-in-the-same-model).
...
3. **Derive LLM tools:** Strip `pdp_action`, `risk_tier`, entitlements ([Manifest registry § manifest → LLM](/playbooks/agents/manifests/manifest-registry#manifest--llm--pep)).
4. **On proposal:** Validate against pinned manifest; log `manifest_version`; route to PEP.

Boundary detail: [Agentic app](/playbooks/governance/runtime/boundary/agentic-app).

## Release and rollback

| Event | Action |
| --- | --- |
| **New tool added** | Bump manifest version; add [policy scenarios](/playbooks/governance/runtime/assurance/policy-test-scenarios); CI must pass |
...
Align manifest promotion with [PGAR blueprint release gate matrix](/blueprints/governance/runtime#release-gate-matrix): tool / ACL changes re-run Tool + Action evals.
...
| Debug flag exposes all tools | Bypass manifest; [Adversarial testing](/playbooks/governance/runtime/assurance/adversarial-testing) must catch |
...
See: [Manifest registry](/playbooks/agents/manifests/manifest-registry) · [Agents Blueprint](/blueprints/agents-blueprint) (manifests) · [MCP Blueprint](/blueprints/mcp-blueprint) (transport) · [Boundary: Agentic app](/playbooks/governance/runtime/boundary/agentic-app) · [Audit & replay](/playbooks/governance/runtime/foundation/audit-and-replay)
