...
**LangChain** gives you models, tools, prompts, and linear composition. **LangGraph** gives you the cyclic, stateful orchestration that production agents need. You use both: LangChain components inside LangGraph nodes, with **your platform** owning routing, policy, validation, memory tiers, and LLM selection.

| Need | Use |
| --- | --- |
| RAG Q&A, extraction, classification | LangChain chain |
| Demo agent, internal tool, short loop | LangChain `AgentExecutor` |
| Governed multi-step loop, step-up, resume | LangGraph |
| Intent routing, PEP, eval gates | Your agentic app — before and around either |

Build the loop you can **eval, trace, and audit** — not the loop that ships fastest in a notebook.
