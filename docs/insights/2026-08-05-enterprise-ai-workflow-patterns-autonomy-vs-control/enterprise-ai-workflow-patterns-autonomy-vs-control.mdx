...
This is a **decision guide**: Pattern 0 (single inference), Pattern 1 (fully autonomous), Pattern 2 (deterministic), and Pattern 3 (guided hybrid), with comparison criteria and JSON-shaped contracts. The reference design is the [Autonomy Blueprint](/blueprints/autonomy-blueprint). It builds on [How to Design an Intent Router](/insights/design-intent-router) and [Policy-Governed Agent Runtime](/insights/policy-governed-agent-runtime).
...
Use this section to pick Pattern 0-3. Everything after it (memory, ingress, agent-to-agent, split) is orthogonal: it applies once you have a pattern, and does not change which pattern you chose.
...
Memory is **not** a workflow step on the route row. The route (or its `policy_profile`) declares **whether** memory is allowed and under what isolation / TTL rules. The [agentic app](/playbooks/pgar-runtime/boundary/agentic-app) owns read, write, and assembly during the session. Treat memory like manifests and prompts: **config and runtime**, not another tool the LLM invents mid-path.
...
**Long-term and episodic memory are retrieval**, not free context. Load them in [context assembly](/insights/what-is-agentic-loop) or via an allowlisted tool on the manifest. Do not dump every past session into the system prompt.

#### Route shape (optional)

Routes stay lean. Prefer a `memory_profile` (or fields on `policy_profile`) over embedding store credentials or raw history rules in the row.
...
```json
{
  ...
  "intent": "contract_review",
  "tool_manifest": "contract_review_staged_v3",
  "workflow_id": "contract_review_stages_v3",
  "policy_profile": "read_only_standard",
  "memory_profile": {
  ...
  "prompt_id": "contract_review_host_v3",
  "eval_suite_id": "contract_review_golden"
}
```
...
When the row's `memory_profile` (or memory fields on `policy_profile`) change, treat it like any other route-row policy change: bump and pin per the [route table lifecycle](/playbooks/router/intent-router/route-table-lifecycle).
...
Patterns 0-3 answer **how much autonomy** a capability gets. They do **not** answer **how many agentic apps** you need. Hundreds of routes, including several Pattern 1 autonomous routes, can share one agentic app. A new use case is usually a new **route**, not a new agent.
...
flowchart TB
    ...
    Router --> R1[Route A<br/>Pattern 0]
    Router --> R2[Route B<br/>Pattern 1]
    Router --> R3[Route C<br/>Pattern 3]
    R1 --> App[Shared agentic app]
    R2 --> App
    R3 --> App
...
Same runtime, different `route_id`. Autonomy lives in the route contract (`tool_manifest`, `max_loop_steps`, model profile), not in a separate process.
...
| **Different risk / compliance** | Payment or write-path isolation needs a harder boundary than a route allowlist |
...
- Do **not** create an agent per intent, chatbot, or Pattern 1 route.
- Do **not** equate "autonomous" with "separate agentic app." Multiple autonomous routes on one runtime is the default.
...
:::info[Related]
For the deeper routes-vs-agents decision guide, see One Agent with Routes vs Specialized Agents (coming soon).
:::

### Route versioning

All four patterns share one versioned **route table**. Workflows, manifests, prompts, schemas, and eval suites are separate artifacts the route references. Bump and session-pin rules are the same regardless of Pattern 0-3. For when to bump `route_table_version` and what to pin mid-flight, see [How to Design an Intent Router](/insights/design-intent-router) and [Route table lifecycle](/playbooks/router/intent-router/route-table-lifecycle).
...
:::info[Builds on]
[Autonomy Blueprint](/blueprints/autonomy-blueprint) · [What Is an Intent Router](/insights/what-is-intent-router) · [How to Design an Intent Router](/insights/design-intent-router) · [Route table lifecycle](/playbooks/router/intent-router/route-table-lifecycle) · [Policy-Governed Agent Runtime](/insights/policy-governed-agent-runtime) · [MCP for Enterprise Business Agents](/insights/mcp-for-enterprise-business-agents) · [G.A.I.N Agents](/frameworks/gain-agents)
:::
