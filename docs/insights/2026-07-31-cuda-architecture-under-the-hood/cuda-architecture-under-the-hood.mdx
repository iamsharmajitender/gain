...
| **Library stack** | cuBLAS, cuDNN, CUTLASS, NCCL, Thrust, **RAPIDS** (cuDF, cuML, cuGraph), and the frameworks above them | Where the performance actually comes from. Almost nobody writing AI or analytics code writes raw kernels |

That third row is the part worth internalising as an architect. When people say "the CUDA moat", they rarely mean the hardware. They mean that a decade of matrix multiply kernels, convolution algorithms, collective communication routines, **dataframe and classical ML libraries**, and framework integrations are tuned against this one platform. Competing silicon has to reproduce that stack, not just the chip.
...
| Tokenisation and data preprocessing | **CPU**, often; **GPU** for bulk tabular prep | Irregular strings stay on the host; large joins, groupbys, and filters are RAPIDS territory |
| Graph analytics at scale (PageRank, components) | **GPU** with cuGraph when the algorithm parallelises | Classic pointer-chasing walks still hate GPUs; many analytics kernels do not |
...
## RAPIDS: CUDA for dataframes and classical ML

Deep learning is the loud part of the CUDA story. The quieter part, and the one that convinced me the moat is bigger than cuDNN, is **RAPIDS**: open-source libraries that put familiar data-science APIs on the same GPU memory hierarchy this article has been describing.

(NVIDIA has also been folding this suite under the broader **CUDA-X** data-science branding. The library names you will still meet in code and blogs are RAPIDS / cuDF / cuML / cuGraph.)

I had filed "GPU acceleration" as something you only get after rewriting everything as tensors. RAPIDS is the counterexample: keep thinking in tables, models, and graphs; run the heavy loops on the device.

```mermaid
flowchart TB
    subgraph Host["Familiar APIs"]
      PD["pandas-like code"]
      SK["scikit-learn-like code"]
      NX["NetworkX-like code"]
    end

    subgraph Rapids["RAPIDS on CUDA"]
      CUDF["cuDF\nDataFrames in HBM"]
      CUML["cuML\nclassical ML"]
      CUGR["cuGraph\ngraph analytics"]
    end

    subgraph Hw["Same machine as before"]
      HBM["HBM + SMs\nwarps, coalescing, bandwidth"]
    end
...
| Library | Feels like | What it accelerates |
| --- | --- | --- |
| **cuDF** | pandas (and accelerators for pandas / Polars-style workflows) | Load, filter, join, groupby, aggregate on columnar data in GPU memory |
| **cuML** | scikit-learn | Classical ML: linear models, forests, clustering, UMAP-class reductions, and related estimators |
| **cuGraph** | NetworkX-class graph analytics | PageRank, connected components, shortest paths, and related algorithms when they can run in parallel |
| **RMM** | (under the hood) | RAPIDS Memory Manager: pooled device allocations so libraries are not calling `cudaMalloc` on every tiny buffer |
| **Dask-CUDA / dask-cuDF** | Dask on steroids | Same dataframe ideas across multiple GPUs when one card's HBM is not enough |

You still do not write kernels. cuDF's joins and groupbys *are* CUDA kernels, written once by people who care about coalescing. That is the moat pattern again: the value is the tuned library, not your ability to launch a grid.

### A small example: same idea as pandas

<Details summary="cuDF: filter and aggregate on the GPU (pandas-shaped)">

Data lives in device memory. The Python looks familiar on purpose.

```python
import cudf

df = cudf.read_parquet("events.parquet")
hot = df[df["amount"] > 100]
by_user = hot.groupby("user_id").agg({"amount": "sum", "event_id": "count"})
print(by_user.head())
```

</Details>

<Details summary="cuML: train a model with a scikit-learn-shaped API">
...
The point is not the syntax. The point is that **groupbys, joins, and forest training are exactly the kind of regular, bulk work the GPU was built for**, and RAPIDS maps them onto warps and HBM without asking you to think about block sizes.

### Why the earlier lessons still apply

RAPIDS does not escape the architecture. It sits on top of it.

| CUDA lesson from above | How it shows up in RAPIDS |
| --- | --- |
| **Bytes beat FLOPs** | A big join or groupby is mostly moving and comparing columnar data. Bandwidth and layout matter more than peak tensor TFLOPs |
| **PCIe tax** | If you bounce a dataframe to pandas every other line (`df.to_pandas()`), you pay the host round trip and throw away the win |
| **Keep data on device** | The fast path is cuDF → cuML (or cuGraph) without shipping tables back to the CPU between steps |
| **HBM size is real** | A 40 GB GPU does not hold an arbitrary warehouse table. Spill, partition, or use dask-cuDF when the working set does not fit |
| **Irregular work still hurts** | Row-by-row Python UDFs, pathological string logic, and pointer-heavy walks can still be slower on GPU than a tight CPU path |

That last row is the trap I fell into mentally: "GPU dataframes mean everything tabular is faster." Only the parallel, regular parts are. RAPIDS is honest about fallbacks in some accelerator modes; the architecture explains why those fallbacks exist.

### Where RAPIDS fits next to PyTorch

| Path | Typical job | Libraries |
| --- | --- | --- |
| **Analytics / classical ML** | Feature tables, ETL-ish prep, forests, clustering, graph metrics | RAPIDS (cuDF, cuML, cuGraph) |
| **Deep learning** | Training and serving neural nets | PyTorch / JAX / TensorFlow on cuBLAS, cuDNN, NCCL |
| **Combined pipeline** | Build features on GPU, then train a net | cuDF (and friends) → hand tensors to the DL framework, ideally without a host round trip |

They are not rivals. They are different floors of the same building. RAPIDS is how CUDA shows up when your mental model is still a dataframe, not a transformer block.

:::tip[TAKEAWAY]
**RAPIDS is the CUDA moat for people who never write a kernel and never open a neural net tutorial.** Same SMs and HBM; pandas-shaped and sklearn-shaped APIs; the performance still comes from keeping bulk, regular work on the device and not ferrying tables across PCIe for sport.
:::
...
| GPU acceleration means deep learning only | RAPIDS puts the same machine under pandas- and sklearn-shaped APIs for tables, classical ML, and graph analytics |
| A GPU dataframe always beats pandas | Only while the work stays parallel, fits (or partitions) in HBM, and avoids constant host round trips |
...
| **RAPIDS / CUDA-X data science** | [RAPIDS documentation](https://docs.rapids.ai/) · [CUDA-X for Data Science](https://developer.nvidia.com/topics/ai/data-science/cuda-x-for-data-science) | cuDF, cuML, cuGraph, and how the data-science libraries sit on CUDA |
