...
description: Agents propose tool calls. Governance decides whether they run. An architecture breakdown of runtime trust boundaries for production agent systems in regulated industries.
...
Enterprise teams, especially in **banking and other regulated industries**, are connecting agents to operational tools: payment rails, core banking APIs, KYC workflows, trade settlement. Most production designs still leave undefined where **token**, **identity**, and **policy** state live during execution. Prompt-based guardrails are not sufficient for governed production until the organization can state, for every tool call: what the model receives, what the application retains, and which component authorizes execution, and produce an audit trail a compliance officer or regulator can replay.
...
:::tip[THE CLAIM]
An agent **proposes** tool calls. Governance **decides** whether they run. Policy in the prompt or the weights is not enforcement: it's a suggestion the model may ignore.
...
- **API Gateway**: receives the request, validates the token via the Identity Provider
...
- **PEP + PDP**: app forwards the proposal; PDP returns a verdict
- **Payment Hub**: PEP calls downstream only on **Allow**
...
flowchart TB
    ...
    subgraph policy["Policy Layer: rules stay here"]
        direction LR
...
Read it top to bottom: **user → gateway → identity → agentic app → LLM proposes → PEP asks PDP → service executes**. Most agent security stops at the Agentic App boundary and never builds a real PEP + PDP or service-side re-auth. The rest of this piece walks that path: why prompt guardrails fail, then one request traced end to end with the contracts underneath.

:::tip[READ THE PATH]
**User → Gateway → IdP → Agentic App → LLM proposes → PEP asks PDP → Payment Hub executes.** Token and policy never cross the LLM boundary. Every tool call gets a verdict before it touches downstream.
:::
...
## Corporate wire: one request through five gates
...
:::tip[STEP-UP IS A PDP VERDICT]
The model was never wrong for proposing $47,500: it was never given the $25,000 auto-approval limit. **Step-up** is a PDP verdict, not a model feature: the PEP surfaces it, the Agentic App owns the four-eyes approval UX, and only a subsequent **Allow** lets the PEP forward to the Payment Hub. The PEP wasn't correcting the model; it was enforcing a verdict on an action the LLM has no business authorizing. That approval attestation is what lands in the compliance archive: who authorized the exception, against which policy version, before a single dollar moved.
:::
...
The sequence diagram is the story. These payloads are the contracts each hop implements.

:::tip[THE PDP CONTRACT]
...
:::tip[THREE VERDICTS ONLY]
The PDP runs one policy surface. Three outcomes: **ALLOW**, **DENY**, **STEP_UP**. No "the model said it was fine."
:::
...
This also connects to the rest of the stack:

:::tip[HOW PGAR FITS THE STACK]
- **Frozen model, external control**: intelligence in the LLM, control in the PEP + PDP. PGAR is the control surface for *actions*.
- **Truth outside the model**: the generator proposes; the PDP decides. Same split, applied to tool calls.
- **Observable, auditable runs**: every PEP/PDP verdict is an audit-grade event, not a sampled trace.
:::

:::tip[TAKEAWAY]
**The LLM proposes. The Agentic App holds the token. The PDP decides. The PEP enforces. Downstream services re-authorize.** That is PGAR. Governance as architecture, not as a paragraph in the system prompt.
:::
