...
|---|---|---|
| [Before](/insights/before-training-an-llm) | Decide the blueprint | Architecture dials set the shape, tokenizer builds the vocabulary, and parameter count is fixed. |
| [During](/insights/during-training-an-llm) | Fill in the values | Random weights become meaningful through training: a four-step loop run millions or trillions of times. |
| [Alignment](/insights/aligning-an-llm) | Make it helpful | Show good examples (SFT) and teach which answers are better (RLHF/DPO). |
| [After](/insights/after-training-an-llm) | Run it, read-only | Weights are frozen (no learning); inference traverses the model geometry one token at a time. |
...
Deep dive: [Before Training an LLM: From Design Dials to a Frozen Shape](/insights/before-training-an-llm): hyperparameter math, 7B parameter walkthrough, context window, and tokenizer steps.
...
Deep dive: [During Training an LLM: From Random Weights to a Working Model](/insights/during-training-an-llm): the four-step loop, what each component learns, and the pre-trained base before alignment.
...
Deep dive: [Aligning an LLM: From Autocomplete to Assistant](/insights/aligning-an-llm): SFT, preference tuning (RLHF/DPO), and why alignment is a thin behavior layer.
...
Deep dive: [After Training an LLM: From Frozen Weights to Token-by-Token Inference](/insights/after-training-an-llm): frozen weights, prefill vs decode, sampling, KV cache, and serving tricks.
