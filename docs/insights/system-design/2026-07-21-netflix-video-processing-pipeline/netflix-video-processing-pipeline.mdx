...
| --- | --- | --- |
| **1. Ingest** (mezzanine / IMF) | Accept the high-quality studio source package | Garbage in, expensive garbage out |
| **2. VIS** (Video Inspection Service) | Validate conformance and basic source quality | Bad masters pollute every ladder rung |
| **3. VES** (Video Encoding Service) | Chunked parallel encode into codec/bitrate ladders | Latency and cost explode; devices cannot adapt |
| **4. VQS** (Video Quality Service) | Score encodes (e.g. VMAF); feed analytics | Blind bitrates; bad viewer experience |
| **5. Packaging** | Segment + manifest for HLS/DASH (plus audio/text) | Players cannot switch quality |
| **6. Open Connect** (OCA edges) | Pre-position assets; serve ABR near the member | Origin melts; startup and rebuffer suffer |
| **7. Player** | ABR: pick the next chunk from buffer and bandwidth | No playback or constant rebuffer |
...
## 4. Quality as a first-class service (VQS)
...
## 5. Audio, timed text, and packaging
...
## 6. Global delivery: Open Connect
