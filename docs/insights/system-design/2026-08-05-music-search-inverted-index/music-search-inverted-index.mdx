...
Keystroke `bli`: suggest postings → candidates → rank → **hydrate from catalog** (artwork and display strings, not index-only ghosts). Uses suggest (edge n-grams), not fuzzy.
...
```
  ...
  <TabItem value="screenshot" label="Screenshot">
...
Full submit path: analyze → exact (or fuzzy on typo) → score with field boosts → stage-1 top-N → stage-2 locale/taste/market → hydrate from catalog. Play resolves `track_id` outside search. Search returns IDs only; catalog and control plane finish the UI and audio.

Screenshot below is a clean submit (`blinding lights`). JSON shows the **typo rescue** variant (`blindng lights`) so you see fuzzy expand when exact miss.

<Tabs groupId="submit-blinding-lights">
  <TabItem value="json" label="JSON (typo path)" default>

```json
{
  "query": "blindng lights",
  "analyzed": ["blindng", "lights"],
  "exact": { "blindng": [] },
  "fuzzy_expand": { "blindng": ["blinding"] },
  "score": {
...
```
  ...
  <TabItem value="screenshot" label="Screenshot (clean submit)">
...
| --- | --- |
| **Catalog + index size** | Shard both by `hash(track_id)` (or routing key); replicate shards for read |
...
Search is sharded the same way as the catalog slice: **`hash(track_id) → shard`**. `t42` and `t91` can live on different shards; each shard only stores postings for its own tracks (`blinding → [t42]` on shard 0, `blinding → [t91]` on shard 1). At query time the coordinator **fans out** to shards, **merges** partial hits, then ranks globally. Indexer events for a track route to the same shard that owns that `track_id`.
