...
The model's prediction is compared against the actual next token in the training data, using the cross-entropy loss. One forward pass over a batch is not one guess: every token position is its own next-token quiz, graded in parallel, then averaged into a single scalar.
...
:::important[One pass, many comparisons]
A sequence of length `T` yields `T` predictions in that one forward pass, not one. Labels are the inputs shifted left by one, so each position is compared to the real token that follows it:
...
| Position sees | Predicts | Compared to |
| --- | --- | --- |
| The | cat | cat |
| The cat | sat | sat |
| The cat sat | on | on |
...
Each position gets its own loss (`-log p(correct next token)`). Those losses are averaged into one number for the whole batch. A `B × T` batch (e.g. about 4 M tokens) means about `B × T` such comparisons per step, then one averaged loss and one weight update. The batch is not re-run word by word.
:::
