...
```mermaid
flowchart TB
    Jane["① Jane<br/>user claims"] --> Agent

    subgraph App["② Agentic app"]
        Agent["③ payments-agent-v2<br/>agent bearer + Jane context"]
    end

    Agent --> Entry

    subgraph Dual["④ Gateway dual check"]
        direction TB
        Entry["Inbound tool call"]
        Entry --> CheckU["⑤ User check<br/>May Jane do this on this resource?"]
        Entry --> CheckA["⑥ Agent check<br/>May this robot call this API?"]
        CheckU --> Both{"Both pass?"}
        CheckA --> Both
    end

    Both -->|Yes| API["⑦ Downstream API"]
    Both -->|No| Stop["Blocked / no call"]
    ...
    classDef ok fill:#dcfce7,stroke:#16a34a,color:#1e293b,stroke-width:2px
    classDef deny fill:#fee2e2,stroke:#dc2626,color:#1e293b,stroke-width:2px

    class Jane,CheckU user
    class Agent,CheckA agent
    class Entry,Both gate
    class API ok
    class Stop deny
```
...
| --- | --- | --- |
| **User** | Jane's login JWT lists every right she might ever need (`accounts:read`, `statements:send`, `payments:initiate`, …) even when she only asked to read one account | Keep her entitlements at ingress. For this call, PDP checks only whether `accounts:read` is in her `emts` for `ACC-4821`. Not listed = deny |
| **Agent** | One fat agent token carries every API scope the client is registered for | After the route loads, mint a token scoped to **this route only** (e.g. statement tools, not payments). Route change mid-session → new downscoped token |
...
sequenceDiagram
    ...
    App->>LLM: Prompt (no tokens)
    LLM-->>App: Propose get_account
    ...
    App->>LLM: Prompt (no tokens)
    LLM-->>App: Propose get_statement_prefs
    ...
    App->>LLM: Prompt (no tokens)
    LLM-->>App: Propose send_statement
...
**Which robot is acting?** Issued **after** the route picks `payments-agent-v2`. Scopes are capped to this route, not Jane's full login and not every API the agent is registered for in the IdP.

Read the token as three answers:

- `client_id`: which software principal is calling; issued by the IdP when this agent is registered (revoke this agent without touching others)
- `scope`: what *this route* may call (`accounts:read`, statement read/send). No payment scopes here even if the IdP registration allows them elsewhere
- `jti`: this token instance (rotate or revoke one mint without killing the whole client)
...
Same agent token for tools 1, 2, and 3 in this session. If Jane's next turn routes to a different contract, mint a new downscoped token; do not widen this one mid-flight.
...
Gateway asks **two** questions. Both must pass or the call is denied.

| Question | Looks at | For tool 3 |
| --- | --- | --- |
| May Jane? | her live `emts` | includes `statements:send` |
...
sequenceDiagram
    ...
    App->>LLM: Prompt (no tokens)
    LLM-->>App: Propose validate_vendor
    ...
    App->>LLM: Prompt (no tokens)
    LLM-->>App: Propose initiate_payment
...
| Jane still allowed **now**? | Take `approved_by` (`jane@bank.example`). Look up her **current** entitlements in the IdP / entitlement store (no live user JWT). Confirm this `action` is still listed. |
...
```json
{
  "action": "payments:initiate",
  "approval_valid": "yes",
  "jane_allowed_now": "yes",
  "payload_unchanged": "yes",
  "agent_allowed": "yes",
  "verdict": "ALLOW"
}
```
