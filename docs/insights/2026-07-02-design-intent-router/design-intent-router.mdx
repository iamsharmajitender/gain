...
This is a **decision guide** for intent router design — route contracts, layered dispatch, uncertainty handling, and eval integration — aligned with [G.A.I.N Agents](/frameworks/gain-agents) and the [Eval Input plane](/playbooks/evaluation/plane-input).
...
- **Gate every routing change** with a golden eval set in CI — same discipline as [Eval Plane ①: Input](/playbooks/evaluation/plane-input).
...
**Source of truth for fields and JSON shapes:** [Route contract reference](/playbooks/agents/intent-router/route-contract-reference) (playbook). Ops (version, promote, rollback): [Route table lifecycle](/playbooks/agents/intent-router/route-table-lifecycle).

Summary of what each row must decide:

| Concern | Typical fields |
| --- | --- |
| Identity | `route_id`, `intent` / `intent_label`, `description` |
| Capability | `tool_manifest`, `model_profile`, optional `workflow_id` |
| Governance | `policy_profile`, optional `retrieval`, optional `memory_profile` |
| Artifacts | `prompt_id`, `output_schema_id`, `eval_suite_id` |
| Bounds | `max_loop_steps`, `fallback`, `required_claims` |

`retrieval.mode` is how RAG sits on the route without changing the autonomy pattern:

| `mode` | Who retrieves | Typical pattern |
| --- | --- | --- |
| omit / `none` | Nobody | Chat, handoff, no knowledge path |
| `deterministic_prefetch` | Agentic app, before the LLM. Model never sees retrieve tools. | [Pattern 0](/blueprints/agents-blueprint) (or Pattern 2 if you name `retrieve → generate` as a fixed workflow) |
| `tool` | LLM may propose retrieve inside `scope`. PEP still gates the action. | [Patterns 1-3](/blueprints/agents-blueprint) |
...
| `policy_qa` | "What is our refund policy?" | Prefetch RAG, no tools |
...
**Logical table, versioned platform config.** Each row is a route contract; storage is an ops choice. Same discipline as [manifest lifecycle](/playbooks/agents/manifests/manifest-lifecycle): version id, CI on change, pin `route_table_version` per session, audit on every decision. **Eligible routes** are not a second store: ingress claims filter the table at request time. Storage patterns and promote/rollback: [Route table lifecycle](/playbooks/agents/intent-router/route-table-lifecycle).

Row fields reference other artifacts by id (`tool_manifest`, `policy_profile`, `memory_profile`); those live in their own versioned stores. Do not put entitlements in the route table; the router intersects claims from ingress with route requirements.
...
| **④ Safety** | Always, after any layer | Injection scan, PII handling, block or force a safe route | Can **veto** a confident classification; [Input plane](/playbooks/evaluation/plane-input) requires **100%** adversarial pass before release |

:::tip[Why session stickiness matters]
Most follow-up turns in a multi-step flow are **continuations**, not new intents. Short replies like "yes", "$500", or "the second one" carry almost no standalone intent signal. Re-classifying every turn invites silent misroutes (for example, `general_chat` for "yes" mid-wire). Stickiness keeps the active `route_id` and partial `entities` in session state so Layer ① resolves continuations **deterministically**, without another classifier call or confidence lottery.
:::
...
The router sits **outside** the plan → act → observe loop. The [agentic app](/playbooks/governance/runtime/boundary/agentic-app) consumes the route contract:
...
4. On `route` → pin `route_table_version` (and `manifest_version` if `tool_manifest` is not `none`)
5. If `retrieval.mode` is `deterministic_prefetch` → agentic app retrieves inside `scope` (PEP-gated), then **one** LLM call with no tool schemas
6. Else if `tool_manifest` is `none` → one LLM call with no tool schemas
7. Else → load pinned **tool manifest** for `route_id`; call LLM with messages + **scoped schemas only**
8. On tool proposal → validate against manifest → PEP → downstream
...
flowchart LR
    ...
    Manifest["Tool manifest<br/>if any"]
    LLM["LLM"]
    PEP["PEP + PDP"]
    ...
    class App,Pin,Prefetch,Manifest,LLM process
```
<br/>

## Step 6 — Build the eval suite in parallel

Do not ship routing without a golden set. Minimum coverage per [Eval Input plane](/playbooks/evaluation/plane-input):
...
Every production misroute becomes a permanent fixture. That is how routing quality compounds — the same principle as eval coverage on the [Eval Input plane](/playbooks/evaluation/plane-input).
...
- [ ] Route table documented per [Route contract reference](/playbooks/agents/intent-router/route-contract-reference) (manifest, policy, model, `retrieval`, `memory_profile` where needed)
...
- **Write the route table before the classifier:** each row binds manifest, policy profile, model path, optional `retrieval`, and eval fixtures.
...
:::info[Builds on]
[Agents Blueprint](/blueprints/agents-blueprint) · [Route contract reference](/playbooks/agents/intent-router/route-contract-reference) · [Route table lifecycle](/playbooks/agents/intent-router/route-table-lifecycle) · [What Is an Intent Router](/insights/what-is-intent-router) · [G.A.I.N Agents](/frameworks/gain-agents) · [Eval Plane ①: Input](/playbooks/evaluation/plane-input) · [PGAR Boundary ②: Agentic App](/playbooks/governance/runtime/boundary/agentic-app)
:::
