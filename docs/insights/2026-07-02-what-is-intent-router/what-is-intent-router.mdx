...
| `retrieval` | Optional. Corpora (`scope`) and whether the app prefetches (`deterministic_prefetch`) or the LLM may propose retrieve (`tool`) |

The router does **not** plan multi-step execution. It does **not** call tools. It does **not** hold credentials. It selects the governed entry point — and the [agentic app](/playbooks/governance/runtime/boundary/agentic-app) takes it from there.
...
This aligns with the [Eval Input plane](/playbooks/evaluation/plane-input): parsing, intent classification, and first-line safety filters happen **before inference begins**. If ambiguous intent or injection passes here, no amount of retrieval quality or tool governance will save the outcome.
...
That is the same discipline as [Eval Plane ①: Input](/playbooks/evaluation/plane-input): behavior validated under uncertainty, not just “did it run.”
...
When you have [specialized agents](/insights/one-agent-routes-vs-specialized-agents) (support, payments, compliance, internal ops), something must decide **which governed path owns the turn**. **Owns the turn** means: for this user message, exactly one route contract is active (workflow, tool manifest, policy profile, and `retrieval` if applicable) before the agent loop starts.
...
| --- | --- | --- |
| “What’s our wire limit for EU corporate clients?” | `policy_qa` | Prefetch RAG (`deterministic_prefetch`), low-risk policy profile, policy corpus. No retrieve tools. |
...
This article defines **what** an intent router is and **why** it belongs in the architecture. The companion piece [How to Design an Intent Router](/insights/design-intent-router) covers route tables, layered classification, confidence thresholds, eval gates, and wiring into the agentic app. When the question is one runtime with routes vs a split into specialized agents, see [One Agent with Routes vs Specialized Agents](/insights/one-agent-routes-vs-specialized-agents).
...
:::info[Builds on]
[G.A.I.N Agents](/frameworks/gain-agents) · [Policy-Governed Agent Runtime](/insights/policy-governed-agent-runtime) · [Eval Plane ①: Input](/playbooks/evaluation/plane-input)
:::
