...
image: ./what-is-agentic-loop.png
tags:
  - ai-intelligence
  - arch
  - agents
...
![Agentic loop: plan, act, observe across governed production stages](./what-is-agentic-loop.png)
...
The ingress boundary is where trust starts. Everything downstream assumes identity is already verified. See [PGAR Boundary ①: Ingress](/playbooks/governance/runtime/boundary/ingress).
...
Evaluated on the [Eval Input plane](/playbooks/evaluation/plane-input): injection resistance, schema validation, PII handling.
...
The [agentic app](/playbooks/governance/runtime/boundary/agentic-app) assembles context. The LLM receives messages and schemas — never the bearer token.
...
STEP_UP is a **PDP verdict**, not a model feature. See [Step-up and attestation](/playbooks/governance/runtime/foundation/step-up-and-attestation).
...
- **Own the loop in the platform:** the model plans and proposes; orchestration executes, validates, persists, and traces every iteration.
- **Diagnose by stage:** misroute at ③, skipped validation at ⑫, and lost step-up at ⑨ are architecture fixes, not prompt tweaks.
- **Validate observations before re-plan or synthesis:** raw tool or retrieval output is not trusted context.
- **Cap iteration explicitly:** max steps, timeouts, and defined exit paths; unbounded loops are a design defect.
- **Trace proposal, PEP verdict, and manifest version per step:** audit must answer who authorized what before money moved.

:::info[Builds on]
[G.A.I.N Agents](/frameworks/gain-agents) · [Policy-Governed Agent Runtime](/insights/policy-governed-agent-runtime) · [What Is an Intent Router](/insights/what-is-intent-router) · [Enterprise AI Workflow Patterns](/insights/enterprise-ai-workflow-patterns-autonomy-vs-control) (Pattern 1 is one of four) · [PGAR Boundary ②: Agentic App](/playbooks/governance/runtime/boundary/agentic-app)
:::
