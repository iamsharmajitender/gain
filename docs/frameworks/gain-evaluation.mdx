...
flowchart LR
    subgraph path["Scoring path"]
        ...
        P["Pipeline<br/>Same stages as prod"]
        V["Validator<br/>Schema · grounding · policy"]
        S["Score<br/>Pass · fail · metrics"]
        G["Gate<br/>Promote · rollback"]
    end

    C --> P --> V --> S --> G
    G -. "feedback · golden sets" .-> P
...
| **Pipeline** | Reproduce production stages on golden inputs | Ad-hoc one-off scripts per team |
