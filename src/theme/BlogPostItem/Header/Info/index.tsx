...
function ReadingTime({readingTime}: {readingTime: number}): ReactNode {
  const readingTimePlural = useReadingTimePlural();
  return <>{readingTimePlural(readingTime)}</>;
}

function DateTime({date, formattedDate}: {date: string; formattedDate: string}): ReactNode {
...
export default function BlogPostItemHeaderInfo({
  ...
  return (
    <div className={clsx(styles.container, 'margin-vert--md', className)}>
      ...
      <ViewCounter pageSlug={permalink} withSpacer />
    </div>
  );
}
